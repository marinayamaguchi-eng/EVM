...
async function syncDatesToInputSheet(dates){
    ...
    try{
        ...
        const inputCol = resp.data.columns.find(c => c.title === INPUT_COL_TITLE);
         if(!inputCol) throw new Error("シートBに '日付｜入力' 列が見つかりません");
         ...
         for(let i = 0; i < resp.data.rows.length; i++){
            updateRows.push({
                ...
                cells:[
                    {
                        columnId: inputCol.id,
                        value: sortedDates[i] || null //
                    }
                ]
            });
         }
         ...
         if(sortedDates.length > resp.data.rows.length){
            const extraDates = sortedDates.slice(resp.data.rows.length);
            const newRows = extraDates.map(d => ({
                toBottom:[
                    {columnId: inputCol.id, value: d}
                ]
            }));
            await axios.post(
