...
async function transposeDates(){
    ...
    */
    ...
    //横に増えた日付列だけ取り出す
    const dateCols = targetSheetResp.data.columns.filter(c => /^\d{1,2}\/\d{1,2}$/.test(c.title));

    //SUM関数(左端～右端まで)
    const sumFormula = `=SUM([${dateCols[0].title}]@row:[${dateCols[dateCols.length-1].title}]@row)`;
