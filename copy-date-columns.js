...
async function transposeDates() {    
    ...
    */
    ...
    //4.ボタンがOFFの列の削除
    const existingDateCols = columns.filter(c => /^\d{2,4}\/\d{1,2}\/\d{1,2}$/.test(c.title));
    ...
    async function sortDateColumns(columns, TARGET_SHEET_ID, headers) {
        ...
        //ⅲ.シート前列を一度並べなおす
        const allColsSorted = [
            ...columns.filter(c => !dateCols.includes(c)), //日付列以外ウィ先に保持
            ...sortedCols //日付列を古い順で追加
        ];

        //ⅳ.並び替えをSmartsheetAPIに反映
        await axios.put(
    ...
    //6.実績列に関数を入力
    const actualCol = targetSheetResp.data.columns.find(c => c.title === '実績');

    //SUM関数(左端～右端まで)
    if (actualCol && sortedCols.length > 0){
    const sumFormula = `=SUM([${dateCols[0].title}]@row:[${sortedCols[sortedCols.length-1].title}]@row)`;

    //全行に対して更新リクエストを作成
    const updateRows = targetSheetResp.data.rows.map(row => ({
        id:row.id,
        cells:[
            {
